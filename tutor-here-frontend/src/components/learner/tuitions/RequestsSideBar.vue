<template>
  ...
  >
    ...
    <div class="m-1" v-if="(items.length == 0 )">
      ...
      >
        ...
        <h6>Tuitions</h6>
        <p>No on-going tuitions or requests.</p>

      </b-card>
    </div>


    <div class="m-1" v-for="item in items">  <!--v-if="(item.status != 2)&&(item.status != 9)&&(item.status != 5)"-->
      <router-link :to="`/dashboard_learner/tuition/${ parseInt(item.idRequest) }`">
        <b-card
            border-variant="light"
